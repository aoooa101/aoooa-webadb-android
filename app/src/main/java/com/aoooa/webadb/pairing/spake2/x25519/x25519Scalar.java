...
package com.aoooa.webadb.pairing.spake2.x25519.x25519;

// x25519_sc
@SuppressWarnings("PointlessBitwiseExpression")
public final class x25519Scalar {
    private static long load_3(final byte[] in, int offset) {
        long result;
    ...
    private static long load_4(final byte[] in, int offset) {
        long result;
        result = in[offset] & 0xff;
        result |= (in[offset + 1] & 0xff) << 8;
        result |= (in[offset + 2] & 0xff) << 16;
    ...
    private static long int64_lshift21(long n) {
        return n << 21;
    }

    /**
     * 256^0*s[0] + 256^1*s[1] + ... + 256^31*s[31] = s mod l
     * where l = 2^252 + 27742317777372353535851937790883648493.
     * <p>
     * This is in-place operation.
     *
     * @param s 256^0*s[0] + 256^1*s[1] + ... + 256^63*s[63]
     */
    public static void reduce(final byte[] s) {
        if (s.length != 64) {
            throw new IllegalArgumentException("The input must be 64 bytes.");
        }
